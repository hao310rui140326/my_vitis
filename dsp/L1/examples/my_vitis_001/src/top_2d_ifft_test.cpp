...
void top_ifft2d(MemWideIFStreamTypeIn& p_inStream, MemWideIFStreamTypeOut& p_outStream) {
#ifdef _DEBUG_TYPES
...
#pragma HLS interface ap_ctrl_none port = return
    ...
    std::cout << "    The Size of 1D Row Kernel                    : " << IFFTParams::N << std::endl;
    std::cout << "    The SSR for 1D Row Kernel                    : " << IFFTParams::R << std::endl;
    std::cout << "    The Transform Direction for Row Kernel       : "
              << ((IFFTParams::transform_direction == FORWARD_TRANSFORM) ? "Forward" : "Reverse");
    std::cout << std::endl;

    std::cout << "    The Size of 1D Column Kernel                 : " << IFFTParams2::N << std::endl;
    std::cout << "    The SSR for 1D Row Kernel                    : " << IFFTParams2::R << std::endl;
    std::cout << "    The Transform Direction for Row Kernel       : "
              << ((IFFTParams2::transform_direction == FORWARD_TRANSFORM) ? "Forward" : "Reverse");
    std::cout << std::endl;
...
#endif
    fft2d<k_memWidth, k_fftKernelSize, k_fftKernelSize, k_numOfKernels, IFFTParams, IFFTParams2, k_rowInstanceIDOffset,
