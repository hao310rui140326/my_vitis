...
int main(int argc, char** argv) {
	...
	static cmpxDataIn l_inMat2[k_fftKernelSize][k_fftKernelSize];
    	...
    	static cmpxDataOut matrix_mult[k_fftKernelSize][k_fftKernelSize];
	...
	//mfft_2d(0,1,l_inMat);
	int  row,col ;
	//mdsp(l_inMat,l_inMat2,matrix_mult,&row,&col);
	...
	//mdsp(l_inMat,l_inMat2,matrix_mult,strm_real_out,strm_imag_out);
	mdsp(l_inMat,l_inMat2,matrix_mult,strm_out);

	//for ( int n = 0; n < FFT_LENGTH; ++n ){		
	//		for (int  m = 0; m < FFT_LENGTH; ++m ){
	//				strm_real_out<<(l_inMat[n][m].real());
	//				strm_imag_out<<(l_inMat[n][m].imag());
	//		}
	//	}

	//strm_max_abs(strm_real_out,strm_imag_out,&row,&col);
	strm_max_abs(strm_out,&row,&col);

	FILE *outfile =fopen("./fft2d_out.txt","w");
	FILE *outfile2 =fopen("./fft2d2_out.txt","w");
	FILE *outfile3 =fopen("./fft3_out.txt","w");
...
for (int idx1=0;idx1<k_fftKernelSize;++idx1)
{
	for (int idx2=0;idx2<k_fftKernelSize;++idx2)
	{
       		...
       		fprintf(outfile3,"%f +j*%f  ",matrix_mult[idx1][idx2].real(),matrix_mult[idx1][idx2].imag());										
	}
       		fprintf(outfile," \n");										
       		fprintf(outfile2," \n");										
       		fprintf(outfile3," \n");										
